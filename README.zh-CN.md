...
- 聚合搜索只用于发现线索，发送前必须打开新闻原文或竞品官方页面核验。
...
| 2. 生成项目 | 创建包含配置、采集、分析、发送、去重和测试的完整 Python 项目。 |
...
- 拥有一个可以配置自定义机器人的钉钉群。
- 拥有当前 GitHub 私有仓库的访问权限。

### 安装到个人 Skill 目录

Codex 会从 `$HOME/.agents/skills` 加载个人 Skill。把本仓库克隆到该目录：
...
| `src/competitor_monitor_bot/` | 采集、分析、钉钉加签、发送和 SQLite 状态管理。 |
...
这些命令只会校验配置或生成预览。真实发送仍受分步流程中的明确确认保护。
...
| 来源 | 原文无法读取、时间超窗或正文不支持标题结论时，删除该条消息。 |
| 内容 | 把核验事实与判断分开；企业自述使用“宣称”“发布”或“推广”等措辞。 |
