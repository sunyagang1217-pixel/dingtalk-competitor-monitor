...
- 通过分步对话确认行业边界、地区、关注动向、竞品、品牌别名、关联主体和优先级。
- 适配任意行业，可同时覆盖国内和海外公开动向，不绑定固定竞品名单。
- Google News、必应搜索、百度搜索、360 搜索和微信公众号搜索五类来源同级采集，不区分主次。
- 微信公众号使用全部竞品名称、别名及关联主体检索，覆盖所有提及文章，不限官方账号，并默认翻取 3 页。
- 全部品牌在五类来源中按名称、已确认别名与母公司/运营主体/负责人等关联词检索，不增加行业词门槛；只命中关联主体时还要求摘要建立品牌关系。
- 记录每个品牌在每个来源上的成功、失败、阻断、部分成功或不适用状态，以及候选数、翻页数和脱敏错误。
- 搜索摘要没有日期的候选也进入核验池，但必须从原文校正真实日期后才能通过预览。
- 只有每个关键品牌达到最低来源覆盖率时才允许空日报；五源默认要求 `priority >= 3` 的品牌至少成功覆盖 3 个来源。
- 完整候选池先核验再按日报条数精选；重点保留人事任免等动态，明确标注未经官方确认的报道。
...
| 阶段 | 结果 |
| --- | --- |
| 1. 定义口径 | 确认行业、地区、竞品、别名、关联主体、优先级、来源覆盖门槛、时间和日报格式。 |
| 2. 生成项目 | 创建包含五源采集、逐品牌来源状态、原文核验、延期队列、发送、去重和测试的完整 Python 项目。 |
...
| 组件 | 用途 |
| --- | --- |
| `config/monitoring.json` | 行业范围、竞品、别名、关联主体、优先级、地区、来源翻页和覆盖门槛配置。 |
...
| `src/competitor_monitor_bot/` | 五源采集、逐品牌来源状态、分析、延期、钉钉加签、发送和 SQLite 状态管理。 |
...
`collect-json` 会输出启用来源、逐品牌逐来源状态、关键品牌覆盖结果和脱敏失败原因。摘要缺少日期的候选使用采集时间占位并标为 `missing`；所有候选只有校正为真实原文日期和受支持的内容类型后才能通过 `analysis-preview`。这些命令不会发送钉钉消息；真实发送仍受分步流程中的明确确认保护。
...
检查使用配置时区的当天，只读 SQLite。`status=sent`、`complete=true` 且退出码 0 才证明本地成功发送记录完整一致；0 条空日报还必须满足关键品牌最低来源覆盖率。缺少记录、发送占位未完成、记录不一致或数据库不可读都会返回失败，不能把聊天显示“完成”当作消息已发送。`send-analysis` 自身也会输出 `delivery_check`，未完成时非零退出；群成员是否阅读不在检查范围内。
...
| 来源 | 五类来源同级；登录或安全验证不绕过。逐品牌记录来源状态，全部来源失败时停发。 |
| 空日报 | 每个关键品牌必须达到配置的最低完整成功来源数；`partial` 不计为完整成功。 |
