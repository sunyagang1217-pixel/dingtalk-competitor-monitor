...
```text
这是自动化 heartbeat 执行，不是普通聊天请求。收到后不要回复“已收到”“请告诉我下一步”或其他确认语。必须先运行项目目录中的 check-result，并根据返回的 phase 和 status 继续执行或结束。
```

已确认是自动化触发时，即使同时出现模型切换提示或会话恢复提示，也应继续执行当前任务；不能用聊天回复代替 `check-result`、`collect-json` 或 `send-analysis`。单独出现环境日期、历史引用、截图或普通聊天并不构成新的发送授权。只有命令输出和持久化结果可以证明本轮完成。

填写项目的最终绝对路径，引用配置文件和以下规则，不填写任何凭据、签名 URL 或真实运行数据。实际 heartbeat 触发后，即使最后附带的是环境日期，也应继续执行该轮任务，不以“收到”、聊天完成或应用标记 completed 当作已完成日报。

建议保持自动化会话用途和模型稳定，将无关工作放到其他会话。模型切换与漏执行同时出现只能作为排查线索，不能据此断定因果。提示词加固不能保证模型每轮都会调用工具，后续仍须核对真实触发与发送记录。

专用会话迁移属于可选的稳定性措施：用户确定新目标会话且工具支持迁移时，更新现有任务的 targetThreadId，保留项目目录、状态库、延期队列、调度与通知偏好，并核验绑定结果。没有可用的新会话标识或模型控制工具时，如实说明尚未迁移或锁定模型，不创建重复任务，不把本会话偷偷改成独立 cron。
